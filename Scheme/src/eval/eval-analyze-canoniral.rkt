...
(define (eval exp env)
  (cond ((self-evaluating? exp) exp);; 1 => 1
        ...
        ((application? exp) (apply* (actual-value (get-operator exp) env);; procedure invocation (a b c) => (apply a (b c)) 
                                    (get-args exp)
                                    env))
        (else (error "Unknown exp type -- EVAL" exp))))

(define (apply* proc args-syntax env)
  ;(print "apply*" proc args-syntax)
  (cond ((primitive? proc) (apply-primitive-procedure proc (eval-args args-syntax env)))
...
(define (trunk-args args-syntax env)
  (if (null? args-syntax)
      '()
      (cons (make-trunk (car args-syntax) env)
            (trunk-args (cdr args-syntax) env))))
;end region

;region LAZY
; ('trunk table)
(define (trunk? obj)
  (and (pair? obj) (eq? (car obj) 'trunk)))

(define (evaluated-trunk? trunk)
  (eq? true (get-trunk-flag trunk)))

(define (make-trunk exp env)
  (let ((table (make-table)))
    (begin ((table 'put) 'exp exp)
           ((table 'put) 'env env)
           ((table 'put) 'evaluated false)
           (list 'trunk table))))

(define (get-trunk-flag trunk)
  (((cadr trunk) 'get) 'evaluated))

(define (get-trunk-exp trunk)
  (((cadr trunk) 'get) 'exp))

(define (get-trunk-env trunk)
  (((cadr trunk) 'get) 'env))

(define (get-evaluated-result trunk)
  (((cadr trunk) 'get) 'result))

(define (set-evaluated-result! trunk result)
  (let ((table (cadr trunk)))
    ((table 'put) 'result result)
    ((table 'put) 'evaluated true)
    ((table 'remove) 'exp)
    ((table 'remove) 'env)))

(define (evaluate-trunk trunk)
  (if (evaluated-trunk? trunk)
      (get-evaluated-result trunk)
      (let ((result (actual-value (get-trunk-exp trunk) (get-trunk-env trunk))))
        (set-evaluated-result! trunk result)
        result)))

(define (actual-value exp env)
  (let ((value (eval exp env)))
    (if (trunk? value)
        (evaluate-trunk value)
        value)))

(define (eval-trunk-wrapper exp env)
  (evaluate-trunk exp))
;end region
...
(put-handler 'trunk eval-trunk-wrapper)
